...
export default function SearchTab() {
  const { memories, addSearchEntry, addActivity, trackEvent } = useApp();
  ...
  const highlightText = (text: string, hits: string[]) => {
    ...
    return parts.map((part, i) =>
      regex.test(part) ? (
        <mark key={i} style={{ background: 'rgba(0,102,255,0.4)', color: '#fff', borderRadius: 2, padding: '0 2px' }}>
          {part}
        </mark>
      ) : (
        part
  ...
  return (
    <>
      <header style={{ marginBottom: 24 }}>
        ...
        <p style={{ opacity: 0.7, fontSize: 14, margin: 0 }}>Find memories using keyword and semantic search.</p>
      </header>
      ...
      {!searching && query && results.length === 0 && (
        <div style={{
          ...
          <p style={{ fontSize: 13 }}>Try different keywords or check your tag filters.</p>
        </div>
      )}
      ...
      {!query && results.length === 0 && (
        <div style={{
          ...
          <i className="fas fa-search" style={{ fontSize: 32, marginBottom: 12, display: 'block' }} />
          <p>Type a query to search your memories.</p>
          <p style={{ fontSize: 13 }}>Cortex uses hybrid keyword and semantic search.</p>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {results.map((r) => (
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                setExpandedId(expandedId === r.memory.id ? null : r.memory.id);
              }
            }}
          ...
          >
            ...
            <p style={{ fontSize: 14, opacity: 0.8, margin: '8px 0', lineHeight: 1.5 }}>
              {highlightText(
            ...
            {expandedId === r.memory.id && (
              <div style={{
                ...
                <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, opacity: 0.9 }}>Why matched</h4>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 13 }}>
                  <div>
                    <span style={{ opacity: 0.6 }}>Keyword hits:</span>{' '}
                    {r.keywordHits.length > 0 ? r.keywordHits.join(', ') : 'None'}
                  </div>
                  <div>
                    <span style={{ opacity: 0.6 }}>Semantic score:</span>{' '}
                    {r.semanticScore.toFixed(3)}
                  </div>
                  <div>
                    <span style={{ opacity: 0.6 }}>Recency boost:</span>{' '}
                    {(r.recencyBoost * 100).toFixed(0)}%
                  </div>
                  <div>
                    <span style={{ opacity: 0.6 }}>Tag boost:</span>{' '}
                    {r.tagBoost.toFixed(3)}
                  </div>
                </div>
                <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      navigator.clipboard.writeText(r.memory.body);
                    }}
                    style={{
                      padding: '6px 12px',
